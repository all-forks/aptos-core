...
impl PeerSetCacheUpdater {
    ...
    pub async fn update(&self) {
        for (chain_id, url) in self.query_addresses.iter() {
            ...
            let result = client.validator_set_validator_addresses().await;
            match result {
                Ok((validator_addresses, state)) => {
                    ...
                    let validator_peers = validator_addresses
                        .iter()
                        .map(|(peer_id, validator_addrs)| {
                            (
                                *peer_id,
                    ...
                    let vfn_peers = validator_addresses
                        .iter()
                        .map(|(peer_id, validator_addrs)| {
                            (
                                *peer_id,
